...
class _AddElderState extends State<AddElder> {
  ...
  void initState() {
    ...
    eldermap = {
      ...
      "phone": "",
      "image":
          "https://www.collinsdictionary.com/images/full/tree_267376982.jpg",
      "mac": "",
    };
  }
  ...
  submit(Map eldermap) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Elder Adding')),
    );
    StoreService.get("token").then((token) => {
          eldermap.putIfAbsent("token", () => token),
          print(eldermap),
          APIService.addElder(eldermap).then((resp) => {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Elder Added')),
                ),
                widget.refresh.call(),
